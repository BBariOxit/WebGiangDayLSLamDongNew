...
export async function submitAttemptByQuizIdSvc(quizId, userId, answers) {
  ...
  const correctByQ = new Map();
  for (const row of allQuestions) {
    if (row.correct_index !== null && row.correct_index !== undefined) correctByQ.set(row.question_id, [Number(row.correct_index)]);
  }
  let totalPoints = 0, earnedPoints = 0; const results = [];
  for (const a of answers) {
    const questionId = a.questionId; const selected = (a.selectedAnswers || []).map(Number);
    const correct = correctByQ.get(questionId) || [];
    const qMeta = allQuestions.find(q => q.question_id === questionId);
    const points = qMeta?.points || 1; totalPoints += points;
    const isCorrect = selected.length === correct.length && selected.every(x => correct.includes(x));
    if (isCorrect) earnedPoints += points;
    results.push({ questionId, isCorrect, selectedAnswers: selected });
  }
